import { drawCssBackground } from "@oc/gradients";
import { getMaskDefinition } from "@oc/masks";
import { incrementCounter } from "@oc/diagnostics/render-perf";
...
import { DEFAULT_GRAPHIC_SOURCE_SIZE } from "@oc/graphics";
...
async function collectNode({
	...
	if (node instanceof EffectLayerNode) {
		...
		items.push({
			type: "sceneEffect",
			// snake_case to match opencut-wasm@0.2.10's SceneEffect deserializer;
			// see the note on FrameItemDescriptor in compositor/types.ts.
