import type { TProject, TProjectMetadata } from "@oc/project/types";
import { getProjectDurationFromScenes } from "@oc/timeline/scenes";
...
import type { SavedSoundsData, SavedSound, SoundEffect } from "@oc/sounds/types";
import {
	migrations,
	runStorageMigrations,
} from "@oc/services/storage/migrations";
import type { Bookmark, SceneTracks, TScene } from "@oc/timeline";
import { roundMediaTime } from "@oc/wasm";
...
class StorageService {
	...
	// The versioning layer (src/opencut/versioning) snapshots and restores the
	// exact serialized project record that saveProject/loadProject round-trip.
	// These expose that raw record without going through the TProject <-> Date
	// (de)serialization, so a snapshot is a byte-for-byte copy of what persists.
